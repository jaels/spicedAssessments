...
this.hasMoreOscarsThan = function (name) {
     if (this.numOscars>name.numOscars) return true;
            else return false;
        };
            }
var actors = [];
actors[0] = new Actor("Leonardo DiCaprio", 41, 1);
actors[1] = new Actor("Jennifer Lawrence", 25, 1);
actors[2] = new Actor("Samuel L. Jackson", 67, 0);
actors[3] = new Actor("Meryl Streep", 66, 3);
actors[4] = new Actor("John Cho", 42, 0);

function getActorByName (str){
    for (var i=0;i<actors.length;i++)
    {
        if (actors[i].name===str) return actors[i];
    }
}
